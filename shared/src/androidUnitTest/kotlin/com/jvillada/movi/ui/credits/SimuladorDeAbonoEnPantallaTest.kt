...
class SimuladorDeAbonoEnPantallaTest {
    ...
    fun `una cuota mas al 9695 muestra el ahorro y las dos fechas`() {
        ...
        composeRule.onNodeWithText("Te ahorras $593.617 en intereses", useUnmergedTree = true).assertExists()
    ...
    /**
     * **Los dos supuestos, en la hoja.** El de la proyección (la tasa y la cuota de hoy) y el del
     * abono (que el banco acorte el plazo y no la cuota). El segundo además es accionable: es lo
     * que él tiene que pedirle al banco para que esta fecha se cumpla.
     */
    @Test
    fun `la hoja declara los dos supuestos`() {
        montar(listOf(libreInversion))
        abrirElSimulador()
    ...
    fun `un abono que no alcanza lo dice en vez de prometer un ahorro`() {
        ...
        composeRule.onNodeWithText("Harían falta $2.549.401 para que se termine", substring = true, useUnmergedTree = true).assertExists()
     ...
     * le atribuiría cientos de millones de ahorro propio. Ver [saleDeTuBolsillo].
     */
    @Test
    ...
    /** Y en uno que sí paga él, ese aviso no está: sería una advertencia sobre nada. */
    @Test
    fun `en un credito propio no aparece el aviso de ahorro ajeno`() {
        montar(listOf(libreInversion))
        abrirElSimulador()
    ...
    fun `saldar el crediagil dice cuanto sobra`() {
        ...
        composeRule.onNodeWithText("Con esto la saldas hoy y te ahorras $157.449 en intereses", useUnmergedTree = true).assertExists()
