...
- **Railway auto-deploy is OFF (since 2026-09-13, to save money): merging to master does NOT deploy.** Deploy explicitly, in batches, from **a git-free export** of `origin/master`: `git archive origin/master | tar -x -C <dir>`, then in `<dir>` run `railway link -p a0e6fd4a-60e8-479b-bf11-70f10a8991b9 -e production -s movi-project` once, `railway variable set MOVI_COMMIT_SHA=<sha> --skip-deploys -s movi-project` (so `/version` can report it — a CLI upload has no `RAILWAY_GIT_COMMIT_SHA`), and `railway up --service movi-project --environment production --detach` (`railway.toml`, builder = Dockerfile). **Never `railway up` from a git worktree: it uploads the repository's MAIN checkout, whatever commit that is** (2026-09-14: two SUCCESS deploys shipped #202 while `/version` reported the new sha). `/version` only proves the variable; check that the build log shows the Dockerfile's current step count, and that `/composeResources/com.jvillada.movi.resources/font/space_grotesk_regular.ttf` returns a TTF, not the SPA's index.html. Production: `https://movi-project-production.up.railway.app`.
