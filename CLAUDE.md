...
- **Railway auto-deploy is OFF (since 2026-09-13, to save money): merging to master does NOT deploy.** Deploy explicitly, in batches, from a clean checkout of `origin/master`: `railway variable set MOVI_COMMIT_SHA=<sha> --skip-deploys -s movi-project` (so `/version` can report it — a CLI upload has no `RAILWAY_GIT_COMMIT_SHA`), then `railway up --service movi-project --detach` (`railway.toml`, builder = Dockerfile). Production: `https://movi-project-production.up.railway.app`.
